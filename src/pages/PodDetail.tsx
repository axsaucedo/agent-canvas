...
import { ArrowLeft, Boxes, RefreshCw, Download, Terminal, AlertCircle, Copy, Check } from 'lucide-react';
...
export default function PodDetail() {
  ...
  const containerInfo = pod.spec.containers.find(c => c.name === (selectedContainer || containers[0]));
        ...
        <div className="container flex h-16 items-center justify-between px-4">
          <div className="flex items-center gap-4">
            <Button
              variant="ghost"
              size="icon"
              onClick={() => returnPath ? navigate(returnPath) : navigate('/')}
              className="shrink-0"
            >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-semibold">{pod.metadata.name}</h1>
      ...
      <main className="container px-4 py-6">
        <Tabs value={currentTab} onValueChange={setCurrentTab} className="space-y-6">
          ...
          <TabsContent value="yaml">
            <YamlViewer resource={pod} title={`Pod: ${pod.metadata.name}`} />
          </TabsContent>
        </Tabs>
      </main>
    </div>
